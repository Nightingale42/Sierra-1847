...
public class D3P2Caminteract : MonoBehaviour
{
    public bool TalkToActualFriend = false;
    public bool TalkToRedFriend = false;

    public LookAt4P2 LookAtScript;
    ...
    IEnumerator TalkToFriendCO()
    {
        ...
        if (D3P2EatingSystem.SliceEaten > 1)
        {
            if (sleepSystem != null)
        ...
        SubText.text = "Friend: ";
        holder = "We try our best to survive until the snow melts.";
        foreach (char c in holder)
        {
            SubText.text += c;
            yield return new WaitForSeconds(time);
        ...
        yield return MousePress();
         ...
         SubText.text = "Friend: ";
        holder = "And at the very least we have dinner for tonight.";
        foreach (char c in holder)
        {
            SubText.text += c;
            yield return new WaitForSeconds(time);
