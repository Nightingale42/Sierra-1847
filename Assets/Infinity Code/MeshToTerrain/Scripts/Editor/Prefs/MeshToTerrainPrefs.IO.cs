    ...
    public partial class MeshToTerrainPrefs
    {
        ...
        private List<GameObject> LoadPref(string id, List<GameObject> defVals)
        {
            ...
            if (EditorPrefs.HasKey(key))
            {
                ...
                for (int i = 0; i < count; i++) retVal.Add(MeshToTerrainUtils.EntityIdToObject(EditorPrefs.GetInt(appKey + id + "_" + i)) as GameObject);
                return retVal;
            }
            return defVals;
        }
        ...
        private List<Terrain> LoadPref(string id, List<Terrain> defVals)
        {
            ...
            if (EditorPrefs.HasKey(key))
            {
                ...
                for (int i = 0; i < count; i++) retVal.Add(MeshToTerrainUtils.EntityIdToObject(EditorPrefs.GetInt(appKey + id + "_" + i)) as Terrain);
                return retVal;
            }
            return defVals;
        }
        ...
        private GameObject LoadPrefGameObject(string id, GameObject defVal)
        {
            ...
            GameObject go = MeshToTerrainUtils.EntityIdToObject(goID) as GameObject;
            return go ?? defVal;
        }
